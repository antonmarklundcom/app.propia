...
- Add `scripts/verify-api-v1.ts` and the `.well-known` templates (plan §5.3.3–4).
...
- `GET /api/v1/listings`, `/api/v1/listings/:publicId`, `/api/v1/meta` return contract-shaped
  JSON with the contract's `Cache-Control`; a draft `publicId` returns 404; bursts hit 429.
...
- PR merged; after deploy, `curl -sI https://realestateinparaguay.com/api/v1/listings` shows
  200 + cache header; PLAN.md updated (incl. `[YOU]` item for TEAMID/SHA-256).
